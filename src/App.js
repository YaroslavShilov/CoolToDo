...
const getDataBase = async (history) => {
  const dataLists =
    (await JSON.parse(localStorage.getItem("lists"))) || defaultDB["lists"];
  const dataTasks =
    (await JSON.parse(localStorage.getItem("tasks"))) || defaultDB["tasks"];
  const dataColors =
    (await JSON.parse(localStorage.getItem("colors"))) || defaultDB["colors"];
...
function App() {
  ...
  const onEditTask = (listId, taskId, value) => {
    ...
    const newTasks = [
      ...tasks,
      (tasks.find((task) => task.id === taskId).text = value),
    ];
